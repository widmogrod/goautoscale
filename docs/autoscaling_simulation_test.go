...
import (
	...
	. "github.com/widmogrod/goautoscale"
...
func TestAutoScalingVisualize(t *testing.T) {
	...
	if err := pCPU.Save(9*vg.Inch, 4*vg.Inch, "autoscaling_simulation_cpu.png"); err != nil {
		t.Fatal(err)
	}

	if err := pInst.Save(9*vg.Inch, 4*vg.Inch, "autoscaling_simulation_inst.png"); err != nil {
		t.Fatal(err)
	}
}
